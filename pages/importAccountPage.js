...
export class ImportAccountPage {
    ...
    async navigateToImportAccountPage(){
         ...
         await expect(this.clickImportAccount).toBeVisible({timeout: 15000});
    ...
    async verifyImportPageTitle(){
        ...
        await expect(this.importPageTitle).toBeVisible({timeout: 80000});
    }

    async verifyUploadFileFieldDisplayed(){
        await expect(this.uploadFile).toBeAttached({timeout: 80000});
    }

    async verifyCreateNewRecordOptionDisplayed(){
        await expect(this.selectOption).toBeVisible({timeout: 80000});
    }
    ...
    async verifyNextStepOfImportPage(){
        await expect(this.nextStepOfImportPage).toBeVisible({timeout: 30000});
    }

    async verifyStillOnUploadImportFilePage(){
        await expect(this.importPageTitle).toBeVisible({timeout: 20000});
        await expect(this.nextStepOfImportPage).not.toBeVisible({timeout: 30000});
    }
}
